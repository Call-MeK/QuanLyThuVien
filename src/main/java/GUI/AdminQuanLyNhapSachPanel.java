...
public class AdminQuanLyNhapSachPanel extends JPanel {
    ...
    private JPanel buildPanelPhieu() {
        ...
        pnlPhieu.add(lbl("Mã phiếu nhập:", fl));
        pnlPhieu.add(txtMaPN);
        pnlPhieu.add(lbl("Ngày nhập:", fl));
        pnlPhieu.add(txtNgayNhap);
        pnlPhieu.add(lbl("Nhà cung cấp:", fl));
        pnlPhieu.add(cbNXB);
        pnlPhieu.add(lbl("Tổng tiền:", fl));
        pnlPhieu.add(lblTongTien);
        pnlPhieu.add(new JLabel());
        pnlPhieu.add(new JLabel());
        pnlPhieu.add(new JLabel());
        pnlPhieu.add(new JLabel());
        ...
        pnlNVFields.add(lbl("Mã nhân viên:", fl));
        pnlNVFields.add(txtMaNV);
        pnlNVFields.add(lbl("Họ tên:", fl));
        pnlNVFields.add(txtHoTenNV);
        pnlNVFields.add(lbl("Vai trò:", fl));
        pnlNVFields.add(txtVaiTro);
    ...
    private JPanel buildPanelChiTiet() {
        ...
        txtTenSach = roField(fi);
        txtTenSach.setPreferredSize(new Dimension(200, 32));
        txtSoLuong = tf(fi, 6);
        txtSoLuong.setText("1");
        txtDonGia = tf(fi, 10);
        txtDonGia.setText("0");

        pnlAddRow.add(lbl("Mã sách:", fl));
        pnlAddRow.add(txtMaSachNhap);
        pnlAddRow.add(lbl("Tên sách:", fl));
        pnlAddRow.add(txtTenSach);
        pnlAddRow.add(lbl("Số lượng:", fl));
        pnlAddRow.add(txtSoLuong);
        pnlAddRow.add(lbl("Đơn giá 1 cuốn:", fl));
    ...
    private JPanel buildPanelLichSu() {
        ...
        pRight.add(new JLabel("Tìm: "));
        pRight.add(txtSearch);
        pRight.add(btnTim);
        pRight.add(btnXoaLoc);
    ...
    private void initEvents() {
        ...
        btnExport.addActionListener(e -> {
            if (modelLichSu.getRowCount() == 0) {
                JOptionPane.showMessageDialog(this, "Không có dữ liệu Lịch Sử để xuất!", "Cảnh báo",
        ...
        btnSaveExcel.addActionListener(e -> {
            if (modelChiTiet.getRowCount() == 0) {
                JOptionPane.showMessageDialog(this, "Bảng Chi Tiết đang trống, vui lòng Import dữ liệu trước!",
    ...
    private void themDong() {
        ...
        for (int i = 0; i < modelChiTiet.getRowCount(); i++) {
            if (modelChiTiet.getValueAt(i, 0).toString().equals(maSach)) {
                JOptionPane.showMessageDialog(this, "Sách " + maSach + " đã có trong phiếu!", "Cảnh báo",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "Số lượng > 0 và đơn giá phải là số hợp lệ!", "Cảnh báo",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private void capNhatDong() {
        int row = tblChiTiet.getSelectedRow();
        if (row < 0) {
            JOptionPane.showMessageDialog(this, "Chọn 1 dòng trong bảng để cập nhật!", "Cảnh báo",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
        String maSach = txtMaSachNhap.getText().trim();
        ...
        try (java.sql.Connection con = DAO.DatabaseConnection.getConnection()) {
            ...
            try {
                for (int i = 0; i < modelChiTiet.getRowCount(); i++) {
                    ...
                    tongBanSao += copyBUS.insertNewCopies(con, maSach, tenSach, sl);
                }
                con.commit();
            } catch (Exception ex) {
                con.rollback();
    ...
    public void lamMoiPhieu() {
        txtMaPN.setText(pnBUS.generateMaPN());
        txtNgayNhap.setText(LocalDate.now().toString());
        if (cbNXB != null && cbNXB.getItemCount() > 0)
    ...
    private JTextField roField(Font f) {
        ...
        tf.setBackground(new Color(233, 236, 239));
        return tf;
    }
