...
public class PhieuMuonBUS {
    ...
    public PhieuMuonDTO getById(String MaPM) {
        if (listPhieuMuon != null && !listPhieuMuon.isEmpty()) {
            for (PhieuMuonDTO pm : listPhieuMuon) {
                if (pm.getMaPM() != null && pm.getMaPM().equals(MaPM)) return pm;
            }
        }
    ...
    public String insert(PhieuMuonDTO pm) {
        if (pm.getMaPM() == null || pm.getMaPM().trim().isEmpty()) return "Mã Phiếu Mượn không được để trống!";
        if (pm.getMaThe() == null || pm.getMaThe().trim().isEmpty()) return "Mã Thẻ (Độc giả) không được để trống!";
        if (pm.getMaNQL() == null || pm.getMaNQL().trim().isEmpty()) return "Mã Người Quản Lý không được để trống!";
        if (getById(pm.getMaPM()) != null) return "Mã Phiếu Mượn đã tồn tại!";

        if (pmDAO.insert(pm) > 0) {
            if (listPhieuMuon != null) listPhieuMuon.add(pm);
    ...
    public String update(PhieuMuonDTO pm) {
        if (pm.getMaPM() == null || pm.getMaPM().trim().isEmpty()) return "Mã Phiếu Mượn không được để trống!";
        if (getById(pm.getMaPM()) == null) return "Không tìm thấy Phiếu Mượn cần cập nhật!";
    ...
    public String delete(String MaPM) {
        if (getById(MaPM) == null) return "Mã Phiếu Mượn không tồn tại!";
        if (pmDAO.delete(MaPM) > 0) {
            if (listPhieuMuon != null) listPhieuMuon.removeIf(pm -> pm.getMaPM().equals(MaPM));
    ...
    public ArrayList<PhieuMuonDTO> search(String keyword) {
        ...
        if (listPhieuMuon == null || listPhieuMuon.isEmpty()) getAll();
